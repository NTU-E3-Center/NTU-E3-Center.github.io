...
function change101Top() {
  ...
  const utc8Day =
    timeNow.getUTCHours() >= 16 ? timeNow.getUTCDay() + 1 : timeNow.getUTCDay();
  const topColorHex = getComputedStyle(
    document.documentElement
  ).getPropertyValue(colorArray[utc8Day % 7]);
...
function updateHpPlaneText() {
  ...
  hpPlane.style.setProperty(
    "--_plane-fly-distance",
    `${(-hpPlaneFlyDistance / hpImgPxWidth) * 105}%`
  );
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  const allResourcesPromise = Promise.all([
    new Promise((resolve) => {
      updateHpPlaneText();
      resolve("Plane Updated");
    }),
    new Promise((resolve) => {
      change101Top();
      resolve("101 Top Changed");
    }),
...
function renderRain() {
  ...
  const makeItRain = function (ifBack, rainSlope, precip, dropTime) {
    const hpSvgRain = ifBack
      ? document.querySelector(".hp-rain-back")
      : document.querySelector(".hp-rain-front");
    const rainIntensity = (ifBack ? 100 : 1000) / precip;
    let increment = rainSlope < 0 ? 0 : -rainSlope;
    let drops = "";

    while (
      rainSlope < 0 ? increment < 960 + Math.abs(rainSlope) : increment < 1300
    ) {
      ...
      drops += `<line x1="${splatX + rainSlope}" y1="${
        ifBack ? splatY - 570 : 0
      }" x2="${splatX}" y2="${ifBack ? splatY : 570}" style="animation-delay: ${
        animationDelay / secDevide
      }s; animation-duration: ${dropTime / secDevide}s"/>`;

      ifBack
        ? (drops += `<path d="M${splatX - 10},${splatY}A7,5,0,0,1,${
            splatX + 10
          },${splatY}" style="transform-origin: ${splatX}px ${splatY}px; animation-delay: ${
            animationDelay / 1000
          }s; animation-duration: ${dropTime / 1000}s"/>`)
        : null;
    }
  ...
  fetch(weatherApiUrl)
    ...
    .then((data) => {
      ...
      if (precip >= 0.2) {
        ...
        const windDirection =
          windDirectionDeg > faceDirectionDeg &&
          windDirectionDeg < 180 + faceDirectionDeg
            ? 1
            : -1;
...
function animateHpTheSky() {
  ...
  function animate(time) {
    ...
    let newY =
      direction == 1
        ? initialY - deltaY * easeProgress
        : targetY + deltaY * easeProgress;
    let newYStar = newY - initialY;

    if (cycle < 2) {
      hpTheSkyBg.setAttribute(
        "points",
        `379.5 ${newYStar + 143.85} 366.5 ${newYStar + 156.15} 366.5 ${
          newYStar + 143.85
        } 309.71 ${newYStar + 197.58} 309.71 409.15 379.5 409.15`
      );
      hpTheSkyBack.setAttribute(
        "points",
        `374.5 ${newYStar + 155.46} 366.5 ${
          newYStar + 163.03
        } 366.5 404.15 374.5 404.15`
      );
      hpTheSkyFront.setAttribute(
        "points",
        `361.5 ${newYStar + 155.46} 314.71 ${
          newYStar + 199.73
        } 314.71 404.15 361.5 404.15`
      );
    }
...
document.querySelectorAll(".filter").forEach((filter) => {
  ...
  const originalDisplay = getComputedStyle(
    document.querySelector(`.filter-content[where="${where}"]`)
  ).display;

  document
    .querySelectorAll(`.filter-checkbox[where="${where}"]`)
    .forEach((checkbox) => {
      document
        .querySelectorAll(`.filter-content[where="${where}"]`)
        .forEach((elem) => {
...
document
  .querySelectorAll('[class*="-img"]:has(img[loading="lazy"])')
  .forEach((block) => {
    const img = block.querySelector("img");
    function lazyImgLoaded() {
      block.classList.add("lazy-img-loaded");
    }
...
const resBlockWithAni = Array.from(
  document.querySelectorAll(".res-block")
...
const mediaModalContentWrapper = document.querySelector(
...
allMediaBlocks.forEach((block) => {
  block.addEventListener("click", () => {
    ...
    // --- IF AN IMAGE WAS CLICKED (and it's not a YouTube link) ---
      ...
      mediaModalContentWrapper.style.backgroundImage =
