...
- **[IA-1] [M] ✅** Header nav restructured from seven flat links into five items with three dropdowns — About · People ▾ (Members · Students & Alumni) · Research ▾ (Publications / Projects) · News & Life ▾ (News / Group Life) · Contact. People splits into two destinations: Members (`/members/`, PI + Staff) and Students & Alumni (`/students/`) — both plain links, no subnav (in-page group anchors on `/students/` remain, `scroll-margin-top` compensated, but are not surfaced in the dropdown). Decisions and their whys: "Media" was rejected as a group name (collides with the "In the Media" news category); About stays in the nav (logo keeps its universal go-home meaning); Contact stays top-level (conversion target). Grouping is data-driven via `navGroup` / `subnav` fields in `contents/pages.json`; the drawer mirrors groups as eyebrow labels. Dropdown spec in `components.md` § Subpage Header.

- **[IA-2] [M] ✅** About graduated from a homepage anchor (`/#about-us`) to a dedicated `/about/` page — the anchor was the one nav item that behaved unlike its siblings (teleport-and-scroll, never `is-current`, no durable URL for the center's story). The page carries the mission prose (same `articles['about']` source as the homepage section), the group→center history (E3 Group since 2020 → center established 2025, dates verified against the site's own news timeline: "E3 Group" coverage through 2022–2024, "E3 Center" from May 2025), and a **facts strip computed at build time** from the members/publications data so counts never go stale. Nav labels renamed **About** / **Contact** to match the single-word dropdown triggers. The homepage About section stays as a teaser ending in the standard section-CTA. Merging About into Contact was rejected: the form is the JOIN-US conversion target and must not sink below prose.

- **[IA-3] [M] ✅** `/research/` added as "Topics", the first item in the Research dropdown — three pillars (Smart Energy Systems / Low-Carbon Transport & Supply Chains / Sustainable Energy Decision-Making) with flat cluster cards, content in `research.json` (new pillar→cluster schema; the old tag-topic schema and its never-rendered homepage template were retired). Student names deliberately omitted from v1 (several not yet on the roster); descriptions stay at theme level per the manuscripts precedent. The editorial-board article's "research programs" link returned home to `/research/`.
